...
function logConsumption($db, $product_id, $qty, $unit) {
    ...
    $weight = ($effective_unit === 'ea') ? ($qty * $p['weight_per_ea']) : $qty;
    $factor = $weight / 0.1;

    $kj = round($factor * $p['kj_per_100']);
    $protein = round($factor * $p['protein_per_100'], 1);
    $fat = round($factor * $p['fat_per_100'], 1);
    $carb = round($factor * $p['carb_per_100'], 1);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        elseif ($_POST['action'] === 'cook') {
            ...
            // 1. Resolve effective ingredients (Originals or Substitutes)
            ...
            foreach ($base_ingredients as $ing) {
                ...
                if (isset($sub_map[$product_id])) {
                    $product_id = $sub_map[$product_id];
            ...
            // 2. If substitutes used, fork the recipe into a Ghost
            $effective_recipe_id = $used_substitutes 
            ...
            foreach ($final_ingredients as $ing) {
                $required = $ing['amount'] * $multiplier;
                ...
                if ($remaining > 0.001) throw new Exception("Missing stock for product: " . $ing['product_id']);
            }
        ...
        elseif ($_POST['action'] === 'edit_log') {
            ...
            $stmt->execute([$log['product_id']]);
            $p = $stmt->fetch(PDO::FETCH_ASSOC);
            $weight = ($log['unit'] === 'ea') ? ($new_amount * $p['weight_per_ea']) : $new_amount;
            $factor = $weight / 0.1;
            $kj = round($factor * $p['kj_per_100']);
            $protein = round($factor * $p['protein_per_100'], 1);
            $fat = round($factor * $p['fat_per_100'], 1);
